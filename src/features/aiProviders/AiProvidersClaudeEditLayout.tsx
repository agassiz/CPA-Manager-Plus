...
const normalizeCloakConfig = (cloak: ProviderFormState['cloak']) => {
  ...
  return {
    ...
    cacheUserID: cloak.cacheUserID ?? true,
  };
};
...
export function AiProvidersClaudeEditLayout() {
  ...
  useEffect(() => {
    ...
    if (initialData) {
      const seededForm: ProviderFormState = {
        ...
        experimentalCCHSigning: initialData.experimentalCCHSigning ?? false,
  ...
  const isDirty =
    ...
    (baseline.name !== String(form.name ?? '').trim() ||
      ...
      baseline.disableCooling !== (form.disableCooling ?? false) ||
      baseline.experimentalCCHSigning !== (form.experimentalCCHSigning ?? false) ||
      isHeadersDirty ||
      isModelsDirty ||
