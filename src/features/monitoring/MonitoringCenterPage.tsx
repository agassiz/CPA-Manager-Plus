...
export function MonitoringCenterPage() {
  ...
  const [realtimeColumns, setRealtimeColumns] = useState<RealtimeColumnKey[]>(
    initialMonitoringCenterUiState.current.realtimeColumns
  );
  const [realtimeColumnWidths, setRealtimeColumnWidths] = useState<RealtimeColumnWidths>(
    initialMonitoringCenterUiState.current.realtimeColumnWidths
  );
  ...
  const handleRealtimeColumnVisibilityChange = useCallback((columns: RealtimeColumnKey[]) => {
    setRealtimeColumns(columns);
  }, []);

  const handleRealtimeColumnWidthChange = useCallback((key: RealtimeColumnKey, width: number) => {
    setRealtimeColumnWidths((previous) => ({
      ...previous,
      [key]: width,
    }));
  }, []);

  const resetRealtimeColumns = useCallback(() => {
    setRealtimeColumns([...DEFAULT_REALTIME_COLUMNS]);
    setRealtimeColumnWidths({});
  }, []);
  ...
  const dataPanelActions = useMemo(() => {
    ...
    return (
      <RealtimeEventsPanelActions
        ...
        onAccountDisplayModeChange={setAccountDisplayMode}
        onColumnVisibilityChange={handleRealtimeColumnVisibilityChange}
    ...
    handleRealtimeColumnVisibilityChange,
    overallLoading,
    realtimeLogRows.length,
    realtimeColumns,
    refreshAll,
    resetRealtimeColumns,
    scopedFailureCount,
    searchInput,
    t,
    toggleFailedOnly,
  ...
  const handleUsageClear = useCallback(() => {
        ...
        renderContent={(tab) => {
          ...
          return (
            <RealtimeEventsPanel
              ...
              onAccountDisplayModeChange={setAccountDisplayMode}
              onColumnVisibilityChange={handleRealtimeColumnVisibilityChange}
              onColumnWidthChange={handleRealtimeColumnWidthChange}
              onResetColumns={resetRealtimeColumns}
