...
export const buildPrimarySummaryCards = ({
...
}): SummaryCardProps[] => [
  {
    ...
    secondaryValue: `${t('monitoring.success_suffix')} ${formatCompactNumber(
      Math.max(summary.totalCalls - summary.failureCalls, 0)
    )}`,
    secondaryValueTitle: `${t('monitoring.success_calls')} ${formatFullNumber(
      Math.max(summary.totalCalls - summary.failureCalls, 0),
      locale
    )}`,
...
export const buildSecondarySummaryCards = (
  ...
  const cachedTokenMetaParts = [
    ...
    `${t('monitoring.cache_read_tokens_short')} ${formatCompactNumber(cacheReadTokens)}`,
    `${t('monitoring.cache_write_tokens_short')} ${formatCompactNumber(cacheWriteTokens)}`,
  ];
  ...
  return [
    ...
    {
      label: shortLabel(t, 'monitoring.cache_total_tokens_short', 'monitoring.cache_total_tokens'),
      fullLabel: t('monitoring.cache_total_tokens'),
...
export const requestAccountQuota = async (
  ...
  switch (target.provider) {
    case 'antigravity': {
      const quota = await fetchAntigravityQuota(target.file, t);
      const metaLabels =
        quota.creditBalance !== null
          ? [
              `${t('antigravity_quota.credit_label')}: ${t('antigravity_quota.credit_amount', {
                count: quota.creditBalance,
              })}`,
            ]
          : [];
      return {
