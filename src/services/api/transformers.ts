...
export const normalizeConfigResponse = (raw: unknown): Config => {
  ...
  config.disableClaudeCloakMode = normalizeBoolean(
    raw['disable-claude-cloak-mode'] ?? raw.disableClaudeCloakMode
  );
  config.experimentalCCHSigning = normalizeBoolean(
    raw['experimental-cch-signing'] ?? raw.experimentalCCHSigning
  );
