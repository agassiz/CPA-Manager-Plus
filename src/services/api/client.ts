...
class ApiClient {
  ...
  setConfig(config: ApiClientConfig): void {
    this.apiBase = computeApiUrl(resolveRuntimeApiBase(config.apiBase));
  ...
  private resolveRequestUrl(url: string): string {
    ...
    const baseURL = this.apiBase || computeApiUrl(detectApiBaseFromLocation());
  ...
  private setupInterceptors(): void {
    ...
    this.instance.interceptors.request.use(
      (config) => {
        // 设置 baseURL
        config.baseURL = this.apiBase || computeApiUrl(detectApiBaseFromLocation());
        if (config.url) {
  ...
  async fetchRaw(url: string, init: RequestInit = {}): Promise<Response> {
    ...
    if (this.managementKey && !headers.has('Authorization')) {
      headers.set('Authorization', `Bearer ${this.managementKey}`);
    }
    return fetch(this.resolveRequestUrl(url), {
      ...init,
      headers,
    });
  }
