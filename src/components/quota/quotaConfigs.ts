...
> = {
  ...
  canResetQuota: (quota) => quota.rateLimitResetCreditsAvailableCount != null,
...
const renderKiroItems = (
  ...
  if (quota.overageStatus) {
    const enabled = quota.overageStatus.toUpperCase() === 'ENABLED';
    nodes.push(
      h(
        'div',
        { key: 'overage-status', className: styleMap.codexPlan },
        h('span', { className: styleMap.codexPlanLabel }, t('kiro_quota.overage_status')),
        h(
          'span',
          { className: styleMap.codexPlanValue },
...
export const KIRO_CONFIG: QuotaConfig<
  KiroQuotaState,
  Awaited<ReturnType<typeof fetchKiroQuota>>
> = {
  type: 'kiro',
