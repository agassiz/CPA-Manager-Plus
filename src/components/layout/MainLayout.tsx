...
import {
  ConfigSidebarNavigationProvider,
  type ConfigSidebarNavigationState,
} from '@/features/config/configSidebarNavigation';
import {
...
type SidebarMenuMode = 'features' | 'config';
...
export function MainLayout() {
  ...
  const [sidebarMenuMode, setSidebarMenuMode] = useState<SidebarMenuMode>('features');
  const [configSidebarNavigation, setConfigSidebarNavigation] =
    useState<ConfigSidebarNavigationState>(null);
  const [pluginResources, setPluginResources] = useState<PluginResourceEntry[]>([]);
  const contentRef = useRef<HTMLDivElement | null>(null);
  const languageMenuRef = useRef<HTMLDivElement | null>(null);
  const themeMenuRef = useRef<HTMLDivElement | null>(null);
  const headerRef = useRef<HTMLElement | null>(null);
  const previousConfigSidebarAvailableRef = useRef(false);
  ...
  const coreFeatureNavItems: NavItem[] = [
    dashboardNavItem,
    {
      path: '/ai-providers',
  ...
  const pluginManagementNavItems: NavItem[] = supportsPlugin
    ? [
        {
          path: '/plugins',
          label: t('nav.plugins'),
        ...
        },
        {
          ...
          shortLabel: navShortLabel('nav.plugin_store', t('nav.plugin_store')),
          icon: sidebarIcons.pluginStore,
        },
      ]
    : [];
  const pluginResourceNavItems: NavItem[] = supportsPlugin
    ? pluginResources.map((resource) => {
        const pluginLogo = resolvePluginAssetURL(resource.pluginLogo, apiBase);
        const label =
          pluginResources.filter((item) => item.pluginID === resource.pluginID).length > 1
            ? `${resource.pluginTitle} / ${resource.label}`
            : resource.label;
        return {
          path: resource.route,
          label,
  ...
  const operationFeatureNavItems: NavItem[] = [
    ...(featureAvailability.requestMonitoringAvailable
      ...
      : []),
    ...(featureAvailability.modelPricesAvailable
      ? [
          {
            path: '/model-prices',
  ...
  const featureNavSections: NavSection[] = [
    {
      key: 'core',
      label: t('sidebar.feature_group_core'),
      items: coreFeatureNavItems,
    },
    {
      key: 'operations',
      label: t('sidebar.feature_group_operations'),
      items: operationFeatureNavItems,
    },
    {
      key: 'plugin-pages',
      label: t('nav_groups.plugin_pages', { defaultValue: t('nav.plugins') }),
      items: pluginResourceNavItems,
    },
  ].filter((section) => section.items.length > 0);
  const compactNavSections: NavSection[] = [
    { key: 'core', items: [dashboardNavItem, configNavItem, ...coreFeatureNavItems.slice(1)] },
    { key: 'operations', items: operationFeatureNavItems },
  ].filter((section) => section.items.length > 0);
  const navItems = [
    dashboardNavItem,
    configNavItem,
    ...coreFeatureNavItems.slice(1),
    ...operationFeatureNavItems,
  ];
  ...
  const showSidebarModeTabs = showSidebarLabels;
  const configSidebarAvailable =
    Boolean(configSidebarNavigation?.items.length) && showSidebarLabels;
  const showConfigSidebarMenu = showSidebarModeTabs && sidebarMenuMode === 'config';

  useEffect(() => {
    let modeTimer: number | undefined;
    if (configSidebarAvailable && !previousConfigSidebarAvailableRef.current) {
      modeTimer = window.setTimeout(() => setSidebarMenuMode('config'), 0);
    }
    previousConfigSidebarAvailableRef.current = configSidebarAvailable;
    return () => {
      if (modeTimer !== undefined) {
        window.clearTimeout(modeTimer);
      }
    };
  }, [configSidebarAvailable]);

  const handleSidebarConfigMode = useCallback(() => {
    setSidebarMenuMode('config');
    if (currentPath !== '/config') {
      navigate('/config');
    }
  }, [currentPath, navigate]);

  const handleSidebarFeaturesMode = useCallback(() => {
    setSidebarMenuMode('features');
    if (currentPath !== '/') {
      navigate('/');
    }
  }, [currentPath, navigate]);
  ...
  return (
    <ConfigSidebarNavigationProvider value={setConfigSidebarNavigation}>
          ...
          >
            ...
            {showSidebarModeTabs ? (
              <div
                ...
                role="tablist"
                aria-label={t('sidebar.mode_switch', { defaultValue: '侧边栏模式' })}
              >
                <button
                  type="button"
                  role="tab"
                  aria-selected={sidebarMenuMode === 'features'}
                  className={`sidebar-mode-tab ${sidebarMenuMode === 'features' ? 'active' : ''}`}
                  onClick={handleSidebarFeaturesMode}
                >
                  {t('sidebar.mode_features', { defaultValue: '功能' })}
                </button>
                <button
                  type="button"
                  role="tab"
                  aria-selected={sidebarMenuMode === 'config'}
                  className={`sidebar-mode-tab ${sidebarMenuMode === 'config' ? 'active' : ''}`}
                  onClick={handleSidebarConfigMode}
                >
                  {t('sidebar.mode_config', { defaultValue: '配置' })}
                </button>
              </div>
            ) : null}

            <div className={`nav-section ${showConfigSidebarMenu ? 'nav-section-config' : ''}`}>
              {showConfigSidebarMenu ? (
                <div className="nav-menu-section">
                  {configSidebarNavigation?.items.length ? (
                    ...
                    <NavLink
                      ...
                      className={({ isActive }) =>
                        `nav-item nav-config-item ${isActive || currentPath === '/config' ? 'active' : ''}`
                      }
                      onClick={() => setSidebarOpen(false)}
                      title={t('nav.config_management')}
                      data-tooltip={t('nav.config_management')}
                    >
                      <span className="nav-icon">{sidebarIcons.config}</span>
                      <span className="nav-config-copy">
                        <span className="nav-label">{t('nav.config_management')}</span>
                        <span className="nav-config-description">
                ...
                (showSidebarLabels ? featureNavSections : compactNavSections).map(
                  (section, sectionIndex) => (
                    <div className="nav-menu-section" key={`nav-section-${section.key}`}>
                      {sectionIndex > 0 && <div className="nav-menu-divider" aria-hidden="true" />}
                      {showSidebarLabels && section.label ? (
                        <div className="nav-menu-heading">{section.label}</div>
                      ) : null}
                      {section.items.map((item) => (
                        <NavLink
                          ...
                          onClick={() => {
                            setSidebarOpen(false);
                            if (item.path === '/quota' && typeof window !== 'undefined') {
                              window.dispatchEvent(new Event('quota-page-reset-view'));
                            }
                          }}
                          title={item.label}
